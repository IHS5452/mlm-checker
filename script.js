// Get the required elements from the DOM
var companyNameInput = document.getElementById("companyName");
const checkButton = document.getElementById("checkButton");
const resultParagraph = document.getElementById("result");

// Add an event listener to the checkButton
checkButton.addEventListener("click", () => {
  // Get the company name from the input field and convert to lowercase
  const companyName = companyNameInput.value.toLowerCase().trim();  // Reddit Suggestion from LiveCourage334: added this to remove special characters

  // Array of strings to check for MLM
  //Reddit suggestion from kmrkawa, GooGurka, BaskIceBall_is_life for new entries to the MLM list
  const arrayOfStringsToCheck = [
  '31',
  'LR Health & Beauty',
  'Kangen',
    '5linx',
    'PDXStudentWork',
    'abby & anna',
    'acan pacific',
    'acn',
    'actilabs',
    'adornable.u',
    'advocare',
    'aerogrow',
    'agnes & dora',
    'aim global',
    'akasuka',
    'alcone',
    "alice's table",
    "all'asta",
    'allysian sciences',
    'aloe vera of america',
    'aloette',
    'alphay int',
    'alureve',
    'amare global',
    'ambit',
    'amelia james',
    'ameo',
    'american income life',
    'amsoil',
    'amway',
    'ann summers',
    'anorak',
    'anran',
    'apollo',
    'aquasource uk',
    'arbonne',
    'ariix',
    'arsoa honsha',
    'asea global',
    'asirvia',
    'aspire',
    "athena's",
    'atomy',
    'ava anderson',
    'ava rose',
    'avon',
    'b:hip global',
    'bachar nutrition',
    'bamboo pink',
    'barefoot books',
    'bath.ologie',
    'beach body',
    'beachbody',
    'bearcereju',
    'beauticontrol',
    'beauty counter',
    'beauty society',
    'because cosmetics',
    'become international',
    'bedroom kandi',
    'beever',
    'belcorp',
    'bellame',
    'bemer',
    'bioperformance',
    'bod',
    'body by vi',
    'body shop at home',
    'boisset collection',
    'boston finney',
    'bounce life',
    'bud star',
    'burnlounge',
    'buskins',
    'butterfly beauty',
    'cabi',
    'cambridge weight plan',
    'can',
    'captain tortue',
    'carico int',
    'cedipac sa',
    'celebrating home',
    'cellements',
    'chalk couture',
    'chalky & co',
    'chandeal',
    'charle',
    "charlie's project",
    "chef's toolbox",
    'cherish natural products',
    'chloe & isabel',
    'clever container',
    'close to my heart',
    'cloud 9 parties',
    'cocoa exchange',
    'college works',
    'color by amber',
    'color happy',
    'colorstreet',
    'colour me beautiful',
    'compelling creations',
    'conklin',
    'cookie lee',
    'cosway (malaysia)',
    'country scents',
    'country suds',
    'create your life',
    'creative memories',
    'credit repair usa',
    'crunchi',
    'cutco',
    'cvsl',
    'daisy blue naturals',
    'damsel in defense',
    'darceys',
    'dazzle and daze',
    'dc nation',
    'diana',
    'digital altitude',
    'dione cosmetics',
    'direct cellars',
    'discovery toys',
    'divvee',
    'dot dot smile',
    'doterra',
    'du northing designs',
    'dubli network',
    'dudley beauty',
    'dxn',
    'dynamic essentials',
    'ebiz',
    'ecowarehouse',
    'elepreneuer',
    'elevacity',
    'elk river soaps',
    'ella tina',
    'elli kai',
    'elvacity',
    'emgoldex',
    'enagic',
    'endless xpressions',
    'enersource int',
    'enjo',
    'envy jewelry',
    'epicure',
    'equinox international',
    'essential bodywear',
    'european grouping of marketing professionals',
    'european home retail',
    'evanescence network',
    'ever skincare',
    'evolution travel',
    'evolvhealth',
    'faberlic',
    'family first life',
    'fantasia',
    'farmasi',
    'fes connect',
    'fibi & clo',
    'fifth ave collection',
    'first fitness nutrition',
    'fitteam global',
    'flamingo paperie',
    'fleuresse',
    'fm world',
    'for tails only',
    'forever living',
    'forex entourage',
    'fortune hi',
    'four oceans',
    'fragant jewels',
    'freelife',
    'fuel freedom int',
    'fund america',
    'gano excel',
    'gelmoment',
    'gemstra',
    'genesis pure',
    'global affiliate zone',
    'global intergold',
    'global legacy initiative',
    'godesana',
    'gold canyon',
    'golden days',
    'grace & heart',
    'greeting cake company',
    'h2o at home',
    'hanky panky parties',
    "hayward's gourmet popcorn",
    'he(l)o',
    'healthstyle emporium',
    'healthy peach',
    'heavenly chia',
    'heka corp',
    'helo wristbands',
    'hempworx',
    'herbalife',
    'heritage makers',
    'hibody',
    'hinode',
    'holiday magic',
    'home interiors',
    'honey',
    'honey & lace',
    'hualin biotech',
    'icoinpro',
    'id life',
    'igniting passion',
    'imarketslive',
    'in a pikle',
    'income advantage',
    'india hicks',
    'infinitus',
    'initials, inc',
    'inkd up nails',
    'innov8tive nutrition',
    'inteletravel',
    'intimo',
    'isagenix',
    'itworks!',
    'j. elizabeth',
    'j. hilburn',
    'j.r watkins',
    'jafra',
    'jamberry',
    'jamby',
    'jamie at home',
    'janice collection',
    'java momma',
    'javita',
    'jbloom',
    'jequiti',
    'jerky direct',
    'jeunesse',
    'jewel kade',
    'jewelscent',
    'jk apparel',
    'jordan essentials',
    'joymain',
    'joyome',
    'juiceplus',
    'jump natural',
    'kaesar & blair',
    'kalaia',
    'kalo & co',
    'kangen water',
    'kannaway',
    'karat bars',
    'kaszazz',
    'keep collective',
    'keep me safe',
    'keto',
    'keto coffee',
    'ketones',
    'kirby',
    'kleeneze',
    'kobold',
    'kyani',
    "l'bri",
    'labella baskets',
    'lady godiva beauty',
    'lavylites',
    'le',
    'legal shield',
    'legging army',
    'legging girl',
    'lemongrass spa',
    'lereve',
    'lia sophia',
    'life abundance',
    'life leadership',
    'life tree world',
    "life's abundance",
    'lifebrook',
    'lifevantage',
    'lilla rose',
    'limelife',
    'limu',
    'linen world',
    'lipsense',
    'liv international',
    'live sore',
    'longrich',
    'lorraine lee linen',
    'love winx',
    'lr beauty & health',
    'lularoe',
    'lulu ave',
    'luminess',
    'lyoness',
    'm. global',
    'm. network',
    'maelle beauty',
    'magnabilities',
    'magnolia & vine',
    'makeup eraser',
    'man cave',
    'mannatech',
    'mark.',
    'market america',
    'marly ray',
    'marvelous mouse travels',
    'mary & martha',
    'mary kay',
    'marykay',
    'maskara',
    'matilda jane',
    'max & madeleine',
    'maxwell clothing',
    'mca',
    'medifast',
    'melaleuca',
    'metabolife',
    'mia bath and body',
    'mialisia',
    'miche eu',
    'miki',
    'moa nutrition',
    'modere',
    'mojilife',
    'monat',
    'monavie',
    'morinda bioactives',
    'motives cosmetics',
    'multpure',
    'my club 8',
    'my daily choice',
    'my lala leggings',
    'myecon',
    'national safety associates',
    'national wealth center',
    'natura',
    'nature direct',
    "nature's sunshine products",
    "neal's yard remedies organic",
    'neolife',
    'nerium',
    'nevetica',
    'new era',
    'new u life',
    'neway',
    'nikken',
    'noevir',
    'nomades',
    'noonday collection',
    'norwex',
    'nouveau riche',
    'nspire network',
    'nucerity',
    'nui',
    'nuskin',
    'nutrilite',
    'nxivm',
    'omnilife',
    'one hope wine',
    'optavia',
    'opulenza',
    'organo gold',
    'oriflame',
    'origami owl',
    'our hearts desire',
    'paid 2 save',
    'pampered chef',
    'paparazzi',
    'paperly',
    'park lane jewelry',
    'party girl',
    'party lite',
    'party time mixes',
    'partylite',
    'passion parties',
    'pawtree',
    'paycation',
    'peach',
    'pearl chic',
    'peekaboo beans',
    'perfect',
    'perfectly polished',
    'perfectly posh',
    'personally poetic',
    'ph@tt',
    'phatt',
    'php',
    'pink zebra',
    'piphany',
    'pixielane',
    'plexus',
    'plunder',
    'pm international',
    'pola',
    'poofy organics',
    'premier designs',
    'premier financial',
    'premier jewelry',
    'primemybody',
    'primerica',
    'princess house',
    'prodoula',
    'proyoung',
    'pruvit',
    'pulse cosmetics',
    'pure haven',
    'pure romance',
    'purehaven',
    'purely',
    'purium',
    'qivana',
    'qnet',
    'quanjian natural',
    'rain international',
    'rainbow vacuum',
    'real time pain relief',
    'red aspen',
    'red safety',
    'reliv',
    'revitalu',
    'riway',
    'rodan+fields',
    'roland',
    'rolmex',
    'royal tongan limu',
    'royaltie gens',
    'ruby ribbon',
    'saba',
    'sabika jewelry',
    'safegirl security',
    'salad master',
    'sarso',
    'scentsy',
    "schneider's gourmet world",
    'scout & cellar',
    'seacret',
    'sendoutcards',
    'senegence',
    'shakeology',
    'shaklee',
    'shopping sherlock',
    'shrimp & grits',
    'signature homestyles',
    'silpada',
    'silver icing',
    'simple man',
    'simply success elite',
    'simplyfun games',
    'skinny body at home',
    'solavei',
    'solvei',
    'sophie paris',
    'south hill designs',
    'southern living at home',
    'southwestern advantage',
    'sseko',
    'stamped 4 u',
    'stampin up',
    'steam energy',
    'steeped tea',
    'stella & dot',
    'stream energy',
    'style dots',
    'success university',
    'sunrider',
    'sunset gourmet',
    'sunshine empire',
    'surge 365',
    'sweet legs',
    'sweet minerals',
    'symmetry financial group',
    'syntek global',
    'tastefully simple',
    'tavala',
    'tealightful',
    'team national',
    'tedivina',
    'telecom plus',
    'telexfree',
    'the advert platfrom',
    'the body shop at home',
    'the landmark forum',
    'the longabeger company',
    'the super affiliate network',
    'thermomix',
    'thirty one',
    'thirty-one',
    'thirtyone',
    'Nuv*ta',
        'thrive',
    'thrive life',
    'tiber river naturals',
    'tocara',
    'touchstone crystal',
    'touchstone essentials',
    'trades of hope',
    'tranont',
    'transformational beauty',
    'travel evolution',
    'traveling vineyard',
    'traverus global',
    'trivita',
    'tropic skin care',
    'truvision health',
    'ts',
    'tupperware',
    'united sciences of america',
    'united warehouse',
    'univera',
    'usana',
    'usborne',
    'utility warehouse',
    'valentus',
    'vantel',
    'vasayo',
    'vector',
    'vemma',
    'viai la teeth whitening',
    'viaonehope',
    'vibella',
    'vida divina',
    'vie at home',
    'visalus',
    'vitality extracts',
    'vivamk',
    'volo',
    'vorwerk',
    'votre belle maison',
    'voxxlife',
    'wakaya perfection',
    'wakeupnow',
    'watkins inc',
    'wealthperx',
    'wikaniko',
    'wildtree',
    'willing beauty',
    'winasun',
    'wine shop at home',
    'wink naturals',
    'world financial group',
    'world leadership group',
    'world ventures',
    'wotabu',
    'xango',
    'xerveo',
    'xoom energy',
    'xstream travel',
    'xyngular',
    'yanbal int',
    'yandi',
    'yelloow',
    'yevo',
    'yofoto',
    'yoli',
    'yoonla',
    'young living',
    'youngevity',
    'younique',
    'ytb international',
    'zija',
    'zilis',
    'zinzino',
    'zrii',
    'zurvita',
    'zyia',
    'zyn',
    'level',
    'le-vel',
    'ibummerang'
    'master resell rights'
  //other MLMs here
  ];
  
  // check if text field is blank
  if (companyName === '') {
    document.body.style.backgroundColor = "white";
    resultParagraph.textContent = "Search for an MLM";
    return; // Stop further execution of the function
  }
  // Check if the company name contains any of the MLM-related strings
 const getMatchPercentage = (str1, str2) => {
    const commonChars = str1.split('').filter(char => str2.includes(char));
    return (commonChars.length / str1.length) * 100;
  };

  // Check if the company name is a 100% match or a close match to any of the MLM-related strings
  let matchPercentage = 0;
  let closestMatch = '';


//Reddit Suggestion from LiveCourage334: Added potetional match system for the output.
  for (const str of arrayOfStringsToCheck) {
    const currentPercentage = getMatchPercentage(companyName, str);
    if (currentPercentage === 100) {
      document.body.style.backgroundColor = "red";
      resultParagraph.textContent = "Yes, this is an MLM.";
      return; // Stop further execution of the function
    } else if (currentPercentage > matchPercentage) {
      matchPercentage = currentPercentage;
      closestMatch = str;
    }
  }

  // Check if the closest match has a percentage above a certain threshold (adjust this value as needed)
  if (matchPercentage >= 80) {
    document.body.style.backgroundColor = "yellow";
    resultParagraph.textContent = "Maybe. It sounds like an MLM."; // Reddit user suggestion from Crazy-kittycat-lady: Changing the wording of the output
  } else {
    document.body.style.backgroundColor = "green";
    resultParagraph.textContent = "This is NOT an MLM.";
  }
});
